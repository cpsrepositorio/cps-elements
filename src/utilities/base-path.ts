 ...
 * By default, this script will look for a script ending in all.js or autoloader.js and set the base path
  ...
  if (!basePath) {
    ...
    if (configScript) {
      ...
      const fallbackScript = scripts.find(s => {
        return /all(\.min)?\.js($|\?)/.test(s.src) || /autoloader(\.min)?\.js($|\?)/.test(s.src);
      });
      let path = '';
