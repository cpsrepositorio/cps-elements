...
import { registerIconLibrary } from '../../../dist/elements.js';
...
describe('<cps-icon>', () => {
  ...
  describe('defaults ', () => {
    it('default properties', async () => {
      const el = await fixture<CpsIcon>(html` <cps-icon></cps-icon> `);
    ...
    it('the icon is accessible', async () => {
      const el = await fixture<CpsIcon>(html` <cps-icon library="system" name="check"></cps-icon> `);
  ...
  describe('when a label is provided', () => {
    it('the icon has the correct default aria attributes', async () => {
      ...
      const el = await fixture<CpsIcon>(
        html` <cps-icon label="${fakeLabel}" library="system" name="check"></cps-icon> `
      );
  ...
  describe('when a valid src is provided', () => {
    it('the svg is rendered', async () => {
      ...
      const listener = oneEvent(el, 'cps-load');
  ...
  describe('new library', () => {
    ...
    it('runs mutator from new library', async () => {
      const el = await fixture<CpsIcon>(html` <cps-icon library="test-library" name="test-icon1"></cps-icon> `);
      await elementUpdated(el);
  ...
  describe('negative cases', () => {
    ...
    it("svg not rendered with an icon that doesn't exist in the library", async () => {
      const el = await fixture<CpsIcon>(html` <cps-icon library="test-library" name="does-not-exist"></cps-icon> `);
