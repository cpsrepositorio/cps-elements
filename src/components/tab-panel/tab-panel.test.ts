import { aTimeout, expect, fixture, html } from '@open-wc/testing';
import type CpsTabPanel from '../tab-panel.js';

describe('<cps-tab-panel>', () => {
  it('passes accessibility test', async () => {
    const el = await fixture<CpsTabPanel>(html` <cps-tab-panel>Test</cps-tab-panel> `);
    await expect(el).to.be.accessible();
  });

  it('default properties', async () => {
    const el = await fixture<CpsTabPanel>(html` <cps-tab-panel>Test</cps-tab-panel> `);

    expect(el.id).to.equal('cps-tab-panel-2');
    expect(el.name).to.equal('');
    expect(el.selected).to.equal(false);
    expect(el.getAttribute('role')).to.equal('tabpanel');
    expect(el.getAttribute('aria-hidden')).to.equal('true');
  });

  it('properties should reflect', async () => {
    const el = await fixture<CpsTabPanel>(html` <cps-tab-panel>Test</cps-tab-panel> `);

    el.name = 'test';
    el.selected = true;
    await aTimeout(100);
    expect(el.getAttribute('name')).to.equal('test');
    expect(el.hasAttribute('selected')).to.equal(true);
  });

  it('changing selected should always update aria-hidden role', async () => {
    const el = await fixture<CpsTabPanel>(html` <cps-tab-panel>Test</cps-tab-panel> `);

    el.selected = true;
    await aTimeout(100);
    expect(el.getAttribute('aria-hidden')).to.equal('false');
  });

  it('passed id should be used', async () => {
    const el = await fixture<CpsTabPanel>(html` <cps-tab-panel id="test-id">Test</cps-tab-panel> `);

    expect(el.id).to.equal('test-id');
  });
});
