...
export function lockBodyScrolling(lockingEl: HTMLElement) {
  ...
  if (!document.body.classList.contains('cps-scroll-lock')) {
    const scrollbarWidth = getScrollbarWidth(); // must be measured before the `cps-scroll-lock` class is applied
    document.body.classList.add('cps-scroll-lock');
    document.body.style.setProperty('--cps-scroll-lock-size', `${scrollbarWidth}px`);
  }
}
