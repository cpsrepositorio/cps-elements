...
/** Runs a set of generic tests for CPS Elements form controls */
export function runFormControlBaseTests<T extends BaseFormControl = BaseFormControl>(
  tagNameOrConfig:
    | string
    | {
        tagName: string;
...
//
// Applicable for all CPS Elements form controls. This function checks the behavior of:
//   - `.validity`
//   - `.validationMessage`,
//   - `.checkValidity()`
//   - `.reportValidity()`
...
function runAllValidityTests(
  tagName: string, //
  displayName: string,
  createControl: () => Promise<BaseFormControl>
) {
  ...
  describe(`Form validity base test for ${displayName}`, async () => {
    ...
    it('should not emit an `cps-invalid` event when `.checkValidity()` is called while valid', async () => {
      const control = await createControl();
      const emittedEvents = checkEventEmissions(control, 'cps-invalid', () => control.checkValidity());
      expect(emittedEvents.length).to.equal(0);
    });

    it('should not emit an `cps-invalid` event when `.reportValidity()` is called while valid', async () => {
      const control = await createControl();
      const emittedEvents = checkEventEmissions(control, 'cps-invalid', () => control.reportValidity());
      expect(emittedEvents.length).to.equal(0);
    });

    // TODO: As soon as `CpsRadioGroup` has a property `disabled` this
    // condition can be removed
    if (tagName !== 'cps-radio-group') {
      it('should not emit an `cps-invalid` event when `.checkValidity()` is called in custom error case while disabled', async () => {
        const control = await createControl();
      ...
      it('should not emit an `cps-invalid` event when `.reportValidity()` is called in custom error case while disabled', async () => {
        const control = await createControl();
        control.setCustomValidity('error');
        control.disabled = true;
        await control.updateComplete;
...
//
//  Special tests for <cps-button type="button">
//
function runSpecialTests_slButtonOfTypeButton(createControl: CreateControlFn) {
  ...
  it('should not emit an `cps-invalid` event when `.checkValidity()` is called in custom error case, and not disabled', async () => {
    const control = await createControl();
    control.setCustomValidity('error');
  ...
  it('should not emit an `cps-invalid` event when `.reportValidity()` is called in custom error case, and not disabled', async () => {
    const control = await createControl();
    control.setCustomValidity('error');
    control.disabled = false;
    await control.updateComplete;
...
//
// Special tests for <cps-button href="...">
//
function runSpecialTests_slButtonWithHref(createControl: CreateControlFn) {
  ...
  it('should not emit an `cps-invalid` event when `.checkValidity()` is called in custom error case', async () => {
    const control = await createControl();
    control.setCustomValidity('error');
    await control.updateComplete;
    const emittedEvents = checkEventEmissions(control, 'cps-invalid', () => control.checkValidity());
  ...
  it('should not emit an `cps-invalid` event when `.reportValidity()` is called in custom error case', async () => {
    const control = await createControl();
    control.setCustomValidity('error');
    await control.updateComplete;
    const emittedEvents = checkEventEmissions(control, 'cps-invalid', () => control.reportValidity());
...
function runSpecialTests_standard(createControl: CreateControlFn) {
  ...
  it('should emit an `cps-invalid` event when `.checkValidity()` is called in custom error case and not disabled', async () => {
    const control = await createControl();
    control.setCustomValidity('error');
    control.disabled = false;
    await control.updateComplete;
  ...
  it('should emit an `cps-invalid` event when `.reportValidity()` is called in custom error case and not disabled', async () => {
    const control = await createControl();
    control.setCustomValidity('error');
    control.disabled = false;
    await control.updateComplete;
...
// Creates a testable CPS Elements form control instance
async function createFormControl<T extends BaseFormControl = BaseFormControl>(tagName: string): Promise<T> {
...
function checkEventEmissions(control: BaseFormControl, eventType: string, action: () => void): Event[] {
...
// Component `cps-button` behaves quite different to the other components. To keep things simple we use simple conditions
// here. `cps-button` might stay the only component in CPS Elements core behaves that way, so we just hard code it here.
function getMode(control: BaseFormControl) {
  if (
    control.localName === 'cps-button' && //
    'href' in control &&
    'type' in control &&
    control.type === 'button' &&
    !control.href
  ...
  // <cps-button href="...">
  if (control.localName === 'cps-button' && 'href' in control && !!control.href) {
