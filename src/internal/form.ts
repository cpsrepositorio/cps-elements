...
import type CpsButton from '../components/button/button';
...
//
export const formCollections: WeakMap<HTMLFormElement, Set<BaseFormControl>> = new WeakMap();
...
export class FormControlController implements ReactiveController {
  ...
  private attachForm(form?: HTMLFormElement) {
    if (form) {
      ...
      if (formCollections.has(this.form)) {
        ...
        formCollections.set(this.form, new Set<BaseFormControl>([this.host]));
      }
      ...
      // Overload the form's reportValidity() method so it looks at CPS Elements form controls
  ...
  private handleFormData(event: FormDataEvent) {
    ...
    const isButton = this.host.tagName.toLowerCase() === 'cps-button';
  ...
  private reportFormValidity() {
    //
    // CPS Elements form controls work hard to act like regular form controls. They support the Constraint Validation API
    ...
    //
    if (this.form && !this.form.noValidate) {
      // This seems sloppy, but checking all elements will cover native inputs, CPS Elements inputs, and other custom
  ...
  private setUserInteracted(el: BaseFormControl, hasInteracted: boolean) {
    if (hasInteracted) {
  ...
  private doAction(type: 'submit' | 'reset', invoker?: HTMLInputElement | CpsButton) {
    if (this.form) {
  ...
  reset(invoker?: HTMLInputElement | CpsButton) {
    this.doAction('reset', invoker);
  }

  /** Submits the form, triggering validation and form data injection. */
  submit(invoker?: HTMLInputElement | CpsButton) {
  ...
  /**
   * Dispatches a non-bubbling, cancelable custom event of type `cps-invalid`.
   * If the `cps-invalid` event will be cancelled then the original `invalid`
   * event (which may have been passed as argument) will also be cancelled.
   * If no original `invalid` event has been passed then the `cps-invalid`
   * event will be cancelled before being dispatched.
   */
  emitInvalidEvent(originalInvalidEvent?: Event) {
    const slInvalidEvent = new CustomEvent<void>('cps-invalid', {
      bubbles: false,
      composed: false,
      cancelable: true
    });
