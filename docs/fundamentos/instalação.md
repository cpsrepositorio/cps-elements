...
CPS Elements pode ser instalado e configurado de maneiras alternativas, projetadas para diferentes caso de uso, como através do carregamento direto a partir de um [CDN](https://www.jsdelivr.com/package/npm/@cps/web) ou através da instalação local como uma dependência [NPM](https://www.npmjs.com/package/@cps/web).
...
```html
<script type="module" src="https://cdn.jsdelivr.net/npm/@cps/web/all.js"></script>
```
...
<link
  ...
  href="https://cdn.jsdelivr.net/npm/@cps/web/themes/light.css"
/>

<link
  rel="stylesheet"
  media="(prefers-color-scheme: dark)"
  href="https://cdn.jsdelivr.net/npm/@cps/web/themes/dark.css"
...
?> Para maior clareza e simplicidade, as importações de componentes ao longo desta documentação mostração caminhos a partir de `@cps/web` ao invés de um caminho completo. Se você estiver usando CDN, ou uma instalação local sem um resolvedor ou empacotador de módulos, precisará ajustar esses caminhos para apontar para a pasta em que CPS Elements está sendo servido, ou ao caminho completo por CDN.
...
<script type="module">
  import { setBasePath } from '@cps/web/utilities/base-path.js';
...
!> Nunca importe por desestruturação (como em `import { CpsButton } from '@cps/web'`) a partir da raiz dos diretórios, ou a partir de `all.js`. Isto fará com que o navegador carregue toda a biblioteca, anulando a ideia de importação individual. Em vez disso, escolha a dedo os arquivos específicos, como demonstrado acima.
...
export default {
  plugins: [
    copy({
      targets: [{ src: 'node_modules/@cps/web/assets/icons', dest: 'assets' }]
    })
  ]
};
```
